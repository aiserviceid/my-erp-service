...
Status: Roadmap lanjutan Paket Pro setelah Batch 22 Promo Banner Publik.
...
- Service `notificationService.js` sudah punya fungsi kirim ke endpoint Fonnte.
...
- Token masih berpotensi dipakai dari frontend jika tidak dipindah ke backend.
...
Jika syarat belum terpenuhi, tampilkan upgrade/helper:
...
Agar tidak terlihat seperti spam dan tidak membebani provider:

```text
Maksimal 20 nomor per campaign awal
Jeda 3–5 detik antar pesan
```

Batas ini bisa dinaikkan nanti setelah sistem log dan reputasi nomor lebih matang.
...
Tahap awal bisa simpan log ke `tenant.settings.wa_campaign_logs`, tapi untuk produksi sebaiknya pakai tabel khusus.
...
- token bisa terlihat oleh user teknis,
- rawan disalahgunakan,
- sulit audit,
- tidak ada kontrol rate limit dari server.
...
```text
WhatsApp Marketing otomatis via Fonnte
```
...
```text
1. Merge PR #22 — Roadmap Paket Pro
2. Batch 22 — Promo Banner Publik Pro
3. Batch 23 — WhatsApp Marketing Fonnte Automation
4. Test token Fonnte toko
5. Test campaign 1 nomor
6. Test campaign 5 nomor
7. Test campaign 20 nomor
8. Baru promosikan WA Marketing otomatis sebagai fitur Pro
```
